"""
Backdoor Microagent Implementation

This module provides the core microagent implementation for Backdoor AI,
inspired by the OpenHands microagent system but adapted for Python, Flask, and
...
class MicroagentValidationError(Exception):
    """Exception raised when a microagent fails validation."""
    pass
...
class BaseMicroagent(BaseModel):
    ...
    def load(
        cls,
        path: Union[str, Path],
        microagent_dir: Optional[Path] = None,
        ...
        # Legacy repo instructions are stored in .backdoor_instructions
        if path.name == '.backdoor_instructions':
        ...
        if metadata.triggers:
            ...
            # No triggers, default to REPO unless metadata explicitly says otherwise
...
class RepoMicroagent(BaseMicroagent):
    ...
    RepoMicroagents are loaded from `.backdoor/microagents/repo.md` files within repositories
...
def load_microagents_from_dir(
    ...
    Args:
        microagent_dir: Path to the microagents directory (e.g. .backdoor/microagents)
